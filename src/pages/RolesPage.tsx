...
import {
  Shield, Plus, Search, Pencil, Trash2, Users, Key,
  Loader2, AlertCircle, RefreshCcw, ExternalLink, Save, X
} from 'lucide-react';
...
export default function RolesPage() {
  ...
  const totalUsers = result?.data.reduce((acc, r) => acc + Number(r.user_count || 0), 0) ?? 0;
  ...
  return (
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <StatCard label="Tổng vai trò" value={result?.total ?? 0} icon={Shield} color="bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400" />
          <StatCard label="Thành viên gán" value={totalUsers} icon={Users} color="bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400" />
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-white/70 font-semibold uppercase tracking-wider">Thiết lập quyền</p>
                <p className="text-sm font-bold text-white font-outfit">Gán quyền cho Role</p>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {!result?.data.length ? (
                    ...
                    <tr key={r.id} className="group hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                      <td className="px-4 py-3 text-[11px] text-slate-300 dark:text-slate-600 font-mono">{(page - 1) * pageSize + idx + 1}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <p className="font-semibold text-slate-800 dark:text-slate-200 text-sm">{r.name}</p>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <Badge variant="secondary" className="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-0 font-semibold px-2 py-0.5 rounded-md text-[10px]">
                          <Users className="w-3 h-3 mr-1" /> {r.user_count}
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <Badge variant="secondary" className="bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-0 font-semibold px-2 py-0.5 rounded-md text-[10px]">
                          <Key className="w-3 h-3 mr-1" /> {r.permission_codes?.length ?? 0}
                        </Badge>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Guard permission={PERMISSIONS.ROLE_UPDATE}>
                            ...
                            <button onClick={() => navigate('/roles/assign')}
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="p-0 border-0 shadow-2xl overflow-hidden max-w-sm bg-white dark:bg-slate-900">
          <div className="px-6 pt-6 pb-5 text-center space-y-4">
            ...
            <div>
              <DialogTitle className="text-base font-semibold text-slate-900 dark:text-slate-100">Xác nhận xóa vai trò?</DialogTitle>
              <DialogDescription className="text-sm text-slate-500 dark:text-slate-400 mt-1.5">
                Bạn đang xóa <span className="font-semibold text-slate-700 dark:text-slate-300">{deleteTarget?.name}</span>.
